...
abstract class BaseChart<D> extends StatefulWidget {
  ...
  void _updateBehaviors(common.BaseChart chart, BaseChartState chartState) {
    ...
    behaviorList.forEach((ChartBehavior behaviorWidget) {
      // BehaviorCreator bcc =;bcc

      // BehaviorCreator  bc=    ChartBehavior<D>() => behaviorWidget.createCommonBehavior<D>() as ChartBehavior;

      // BehaviorCreator d =ChartBehavior<D> tests<D>()=>;

      final commonBehavior = chart.createBehavior(
        //bc
        <D>() => behaviorWidget.createCommonBehavior<D>() as c.ChartBehavior<D>,
      );
  ...
  bool _notACustomBehavior(ChartBehavior behavior) {
    return this.behaviors == null ||
        !this.behaviors!.any(
                (ChartBehavior userBehavior) =>
            userBehavior.role == behavior.role);
  }

  void _updateSelectionModel(common.BaseChart<D?> chart,
      BaseChartState<D?> chartState) {
...
class LayoutConfig {
  ...
  common.LayoutConfig get commonLayoutConfig =>
      new common.LayoutConfig(
